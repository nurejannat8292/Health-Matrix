...
public class LoginPage extends JFrame {
    ...
    public LoginPage() {
        ...
        passF.setBounds(80, 280, 260, 45); // Restored full width (260)
        passF.setBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, Color.LIGHT_GRAY));
        add(passF);

        showPassBox = new JCheckBox("Show Password");
        showPassBox.setBounds(80, 335, 150, 20); // Placed right beneath the password field line
        showPassBox.setBackground(Color.WHITE);
        showPassBox.setForeground(Color.GRAY);
        showPassBox.setFont(new Font("SansSerif", Font.PLAIN, 12));
        showPassBox.setFocusable(false);
        ...
        showPassBox.addActionListener(e -> {
            ...
            if (currentPass.equals("Password") && passF.getForeground() == Color.GRAY) {
                return;
            }
        ...
        JButton loginBtn = new JButton("Login →") {
            protected void paintComponent(Graphics g) {
                ...
                Color startColor = new Color(74, 222, 128); 
                Color endColor = new Color(34, 197, 94);    
        ...
        passF.addFocusListener(new FocusAdapter() {
            public void focusGained(FocusEvent e) {
                if (new String(passF.getPassword()).equals("Password")) {
                    passF.setText(""); 
                    passF.setEchoChar(showPassBox.isSelected() ? (char) 0 : '●'); 
    ...
    private void performLogin() {
        ...
        if (found) {
            final String finalUserName = matchedName; 
            ...
            okBtn.addActionListener(e -> {
                d.dispose(); 
                new Dashboard(finalUserName).setVisible(true);
                dispose(); 
            });
